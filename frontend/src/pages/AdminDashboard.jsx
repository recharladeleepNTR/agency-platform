...
const AdminDashboard = () => {
  ...
  /* Fetch Portfolio directly from Backend Database API */
  const fetchPortfolioFromDB = async () => {
    try {
      ...
      if (r.data && Array.isArray(r.data)) {
        ...
        setPosts45(items.filter(i => !i.ratio || i.ratio === '4:5' || i.ratio.includes('4:5') || i.ratio.includes('9:16')));
        setThumbnails(items.filter(i => i.ratio === '16:9' || (i.ratio && i.ratio.includes('16:9'))));
        setBanners(items.filter(i => i.ratio === 'Banner' || i.ratio === '6:1' || (i.ratio && i.ratio.includes('Banner'))));
      }
    } catch (e) {
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                {(portfolioSubTab === 'Work Preview' ? workPreviews : portfolioSubTab === '16:9' ? thumbnails : portfolioSubTab === 'Banner' ? banners : posts45).map((item, idx) => {
                  const rawMedia = item.mediaUrl || item.img;
                  let mediaSrc = getMediaUrl(rawMedia);
                  if (mediaSrc && !mediaSrc.startsWith('data:') && !mediaSrc.startsWith('blob:')) {
                    const ts = item.updatedAt ? new Date(item.updatedAt).getTime() : Date.now();
                    mediaSrc = `${mediaSrc}?t=${ts}`;
                  }
